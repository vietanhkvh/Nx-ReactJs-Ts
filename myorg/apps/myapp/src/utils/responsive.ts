...
export const mergeStyle = (firstStyle:any, ...restStyle:any) => {
  const combinedFirstStyle = Array.isArray(firstStyle) ? Object.assign({}, ...firstStyle) : firstStyle;
  const combinedRestStyle = restStyle.map((rest: any) => (Array.isArray(rest) ? Object.assign({}, ...rest) : rest));
  return Object.assign({}, combinedFirstStyle, ...combinedRestStyle);
};

export const combineStyle = ({ MOBILE, DESKTOP, GENERAL }: ICombineStyle) => {
  const switchStyle = {
